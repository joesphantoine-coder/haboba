  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="mb-8">
        ...
        <p className="text-sand-600 mt-2 text-lg">{t('blogs.subtitle')}</p>
      </div>

      {/* Honesty disclaimer — distinguishes this from the cited main archive */}
      <div
        role="note"
        className="bg-sand-100 border border-sand-200 rounded-xl px-4 py-3 mb-10 text-sm text-sand-600"
      >
        {t('blogs.disclaimer')}
      </div>
      ...
      <div className="space-y-10">
        {blogs.map((blog) => (
          <article key={blog.id} className="border-b border-sand-200 pb-8">
            <h2 className="font-serif text-2xl font-semibold text-sand-900">
              {blog.title}
            </h2>
            <p className="text-sm text-sand-500 mt-1">
              {t('blogs.by')} {blog.author}
            </p>
            <p className="text-sand-700 mt-4 leading-relaxed">{blog.content}</p>
          </article>
        ))}
      </div>
    </div>
